...
import com.miya38.connection.okhttp.ApiRequestOkHttp;
...
public abstract class AbstractConnectionCommonOkHttp {
    ...
    private CommonInterface.OnLoaderFinishListerner mOnLoaderFinishListerner;
    ...
    public AbstractConnectionCommonOkHttp(final FragmentActivity activity) {
        mActivity = activity;
        if (activity instanceof CommonInterface.OnLoaderFinishListerner) {
            mOnLoaderFinishListerner = (CommonInterface.OnLoaderFinishListerner) activity;
        }
    ...
    public AbstractConnectionCommonOkHttp(final Fragment fragment) {
        mFragment = fragment;
        if (fragment instanceof CommonInterface.OnLoaderFinishListerner) {
            mOnLoaderFinishListerner = (CommonInterface.OnLoaderFinishListerner) fragment;
        }
        mDisplayProgress = new SparseArray<>();
    }
    ...
    public AbstractConnectionCommonOkHttp(final DialogFragment dialogFragment) {
        ...
        if (dialogFragment instanceof CommonInterface.OnLoaderFinishListerner) {
            mOnLoaderFinishListerner = (CommonInterface.OnLoaderFinishListerner) dialogFragment;
        }
        mDisplayProgress = new SparseArray<>();
    }
    ...
    public void onMethodLoaderFinished(final NetworkRequestOkHttp networkRequestOkHttp, final Response response, final String data) {
        ...
        try {
            if (mOnLoaderFinishListerner != null) {
                mOnLoaderFinishListerner.onLoaderFinished(networkRequestOkHttp, response, data);
            }
